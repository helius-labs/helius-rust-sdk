...
#[tokio::test]
async fn test_search_assets_success() {
    ...
    let request: SearchAssets = SearchAssets {
        owner_address: Some("GdNh12yVy5Lsew9WXVCV5ErgK5SpmsBJkcti5jVtPB7o".to_string()),
    ...
    let response: Result<AssetList, HeliusError> = helius.rpc().search_assets(request).await;
...
#[tokio::test]
async fn test_search_assets_failure() {
    let mut server: Server = Server::new_with_opts_async(mockito::ServerOpts::default()).await;
    let url: String = server.url();
