...
mod tests {
    ...
    fn test_get_collection_metadata_account() {
        ...
        let (expected_pubkey, _bump_seed) = Pubkey::find_program_address(
            &[b"metadata", ID.as_ref(), &collection_mint.to_bytes()],
            &ID,
        );
    ...
    fn test_get_revoke_collection_authority_instruction() {
        ...
        let instruction = revoke_collection_authority_instruction(
            collection_mint,
            collection_authority,
            &revoke_authority_keypair,
        );

        assert_eq!(instruction.program_id, ID);

        let expected_accounts = vec![
            AccountMeta::new(get_collection_authority_record(&collection_mint, &collection_authority), false),
