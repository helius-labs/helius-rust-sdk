...
#[tokio::test]
async fn test_get_multiple_new_address_proofs_v2_success() {
    ...
    let addresses = vec![AddressWithTree {
        address: "addr1".to_string(),
        tree: "tree1".to_string(),
    }];

    let response = helius.get_multiple_new_address_proofs_v2(addresses).await;
...
#[tokio::test]
async fn test_get_multiple_new_address_proofs_v2_failure() {
    let (mut server, helius) = setup_mock().await;
