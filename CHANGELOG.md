...
### Added
- **Pre Confirmations** (`preconfSubscribe`): new standalone `preconf::PreconfClient` for Helius's lowest-latency transaction stream, delivering scheduled transactions over WebSocket before they are shredded. Yields a stream of `PreconfNotification { slot, transaction_index, transaction, transaction_bytes }`, deserializing the bincode `VersionedTransaction` and exposing the raw bytes. Served from the Gatekeeper endpoint (`wss://beta.helius-rpc.com`). Binary frame layout is `slot:u64_le | transaction_index:u64_le | bincode(VersionedTransaction)` (no version field). Credit-based pricing (billed per notification). Includes `PreconfClient::connect`/`connect_with_api_key`/`shutdown`, `PreconfStream`, and `examples/websockets/preconf_subscribe.rs`. A pre-confirmation is an early signal, not a guarantee.
- **Sender Max bundles**: new `send_bundle_with_sender` submits up to 5 transactions to Sender Max via `sendBundle` (`params: [[base64Tx, ...], { encoding: "base64" }]`). The caller includes only the 0.001 SOL Sender tip in ≥1 transaction; Helius adds any pathway tips (including Jito) — callers must not add a separate Jito tip or use Jito tip accounts. Landing is tracked per-transaction by signature (not bundle IDs / `getBundleStatuses`).
