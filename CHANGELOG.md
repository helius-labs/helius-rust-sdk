...
- `GetAssetsByOwner` no longer serializes unset optional fields (`limit`, `before`, `after`, `displayOptions`, `sortBy`, `cursor`) as explicit `null`s in the JSON-RPC `params`; they are now omitted via `skip_serializing_if`, consistent with the sibling request structs.
