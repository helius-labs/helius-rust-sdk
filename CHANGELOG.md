...
### Added
- `CreateSmartTransactionConfig::loaded_accounts_data_size_limit` and the same field on `CreateSmartTransactionSeedConfig` (both with `with_loaded_accounts_data_size_limit` builders), plus `Helius::get_compute_units_with_data_size_limit` / `get_compute_units_thread_safe_with_data_size_limit`. Sets the account-data budget a Transaction v1 carries in its message header, defaulting to `MAX_LOADED_ACCOUNTS_DATA_SIZE_BYTES` (64 MiB) — the implicit budget legacy and v0 transactions get. The limit is applied to the simulated draft as well as the final message, since simulation loads accounts too and an estimate taken under a larger budget does not hold on-chain. `CreateSmartTransactionConfig` also gains `with_v1` (previously only on the seed config), `with_lookup_tables`, `with_fee_payer`, `with_priority_fee_cap`, `with_cu_buffer_multiplier`, and `with_priority_fee_lamports_cap`. **Breaking change (minor in scope):** code constructing either config with a struct literal that enumerates every field must add the new one; literals ending in `..Default::default()` (including every example and test in this repo) are unaffected.
- `MAX_COMPUTE_UNIT_LIMIT` (1,400,000) and `MIN_COMPUTE_UNIT_LIMIT` (1,000) constants, replacing four bare `1_400_000` literals.
- `HeliusError::as_reqwest`, `HeliusError::is_timeout`, and `HeliusError::is_connect`, for classifying a transport failure without downcasting through `source()`. `as_reqwest` returns the underlying `reqwest::Error` for the `Network` and `ReqwestError` variants, giving access to `reqwest`'s own predicates (`is_decode`, `is_body`, `is_redirect`, `status`, ...). `is_connect` covers connection refused, DNS resolution failure, and TLS handshake failure — a request that never arrived, and so is safe to retry whether or not the operation is idempotent. `is_timeout` reports transport timeouts only; a confirmation poll that runs out of time is still `HeliusError::Timeout`, which is a distinct condition. Note that `HeliusError::TlsError` and `SimdJson` still hold only a formatted string, so `as_reqwest` returns `None` for a TLS *configuration* failure; giving those two variants their real errors changes their field types and so is left for the next breaking release.
...
### Fixed
- **A Transaction v1 could be charged a priority fee for compute it was never granted.** The buffered compute-unit estimate was floored at 1,000 but never capped: simulation is limited to 1,400,000 units, so the default 1.25x safety buffer produced up to 1,750,000 for any transaction consuming more than ~1,120,000, and a caller-supplied `cu_buffer_multiplier` could push any transaction over. The runtime *clamps* an over-limit request rather than rejecting it (`solana-runtime-transaction`'s `try_into_config` applies `.min(MAX_COMPUTE_UNIT_LIMIT)` and returns `Ok`), so on legacy/v0 this was harmless — the prioritization fee is derived from the already-clamped limit. On v1 it was not: the total priority fee is carried in the message header and taken **verbatim**, while the SDK derived it from the pre-clamp compute-unit count. A transaction could therefore pay for up to 25% more compute than the 1,400,000 it was actually granted. The requested limit is now capped at `MAX_COMPUTE_UNIT_LIMIT` before the v1 fee is derived from it, across all three builders (`create_smart_transaction`, `create_smart_transaction_with_seeds`, `create_smart_transaction_without_signers`). The existing `MIN_COMPUTE_UNIT_LIMIT` floor behaviour is unchanged, including that the buffer is not applied below it. A non-finite or non-positive multiplier now falls back to the default instead of producing a garbage limit.
- **An unset Transaction v1 compute-unit limit meant a zero compute budget.** `build_v1_config` only set the field when one was supplied, and the runtime resolves a missing v1 limit with `unwrap_or(0)` — so `build_v1_transaction(.., compute_unit_limit: None, ..)` produced a transaction with no compute budget at all. The same footgun as the loaded-accounts limit below, on the sibling field in the same header config. Both budget fields are now always set, defaulting to their maximums, and a supplied value outside `1..=MAX_COMPUTE_UNIT_LIMIT` returns `HeliusError::InvalidInput`.
- **`loaded_accounts_data_size_limit` was silently ignored on legacy and v0.** Those formats have nowhere to carry it, so setting it now returns `HeliusError::InvalidInput` rather than being dropped, mirroring how `lookup_tables` is rejected on v1.
- **A zero loaded-accounts data-size limit was accepted.** `build_v1_transaction` took the limit unvalidated, and SIMD-0385 reads `0` as a zero-byte budget under which every account load fails. As with the compute-unit limit, neither the compiler nor `validate()` catches it. Values outside `1..=MAX_LOADED_ACCOUNTS_DATA_SIZE_BYTES` now return `HeliusError::InvalidInput`.
- **One bad WebSocket frame tore down every subscription on the connection.** `run_ws` owns the single stream and the whole `subscriptions` map, so any `?` or `break` in its message handler drops every subscriber's notification channel at once — and consumers just see their stream end, with no error to explain it. Several server-controlled inputs did exactly that: a non-JSON text frame, an `"id"` that is null or non-numeric (which is what a server returns for a parse or invalid-request error), a response carrying neither `result` nor `error`, a subscribe ack with no subscription id, a duplicate or late ack for an unknown request id, and one consumer dropping its receiver. A single malformed or stale frame from a flaky upstream — or a MITM — killed every live stream the client held.

  Frame handling is now scoped to the frame: an unusable frame is logged and skipped, and a failure that belongs to one waiting request is delivered to that request instead of the whole connection. Only a transport error or a close frame ends the connection. A subscriber that gives up before its ack arrives now has its orphaned server-side subscription released rather than leaked.

  A server error carrying no usable `id` is a special case: it answers one of our requests, but there is no way to tell which, and callers wait on a channel with no timeout. Every in-flight request is failed with that error, while established subscriptions are left untouched. (The old teardown freed those waiters as a side effect of dropping the whole map; keeping the connection alive means releasing them deliberately.)

  So that this does not turn a persistently broken peer into a silent spin, `MAX_CONSECUTIVE_UNUSABLE_FRAMES` (10) still ends the connection with an error after an unbroken run of unusable frames; the counter resets on any frame the client can act on.
...
- **The cause of a network failure was unreachable.** `HeliusError::Network`, `ReqwestError`, and `SerdeJson` wrapped the underlying error but did not mark it `#[source]`, so `std::error::Error::source()` returned `None` and the cause chain dead-ended at the `HeliusError`. `reqwest` 0.12 deliberately omits its own chain from `Display`, so the two together meant a caller learned only *that* a request failed: `Network error: error sending request for url (https://mainnet.helius-rpc.com/)`, with connection refused, DNS failure, TLS handshake failure, and timeout all indistinguishable, and nothing for `anyhow`'s `{:#}`, `eyre`, or `tracing` to print. The wrapped error is now the `source`, so the full chain (`client error (Connect)` → `tcp connect error` → `Connection refused (os error 61)`) is reachable. The `Display` strings are unchanged, so code printing `{}` keeps the message it had and gains the chain on top.
- **Transport failures against Sender and the Jito tip-floor feed were reported as `HeliusError::InvalidInput`.** Five call sites in `optimized_transaction.rs` — the Sender `/fast` send, its response decode, the tip-floor fetch and decode, and the Sender `/ping` warm-up — stringified the `reqwest::Error` into `InvalidInput`, so a refused connection to Sender surfaced as `Invalid input: Sender request error: ...`. That both discarded the cause (as above) and put a network fault in the variant a caller matches to detect its *own* bad arguments. All five now return `HeliusError::Network` with the error attached. Sender HTTP failure statuses likewise went to `InvalidInput`; they now go through `HeliusError::from_response_status`, so a Sender rate limit is `RateLimitExceeded` and an expired API key is `Unauthorized`, as they already were on every other endpoint. Those errors identify the endpoint by host and path (`slc-sender.helius-rpc.com/fast`) rather than by path alone, so the failing *region* is visible; the query string stays out of the message, as it does everywhere else, since that is where an API key would sit.
- **`HeliusError::UrlParseError` printed no detail.** Its `Display` was the fixed string `Url parse error`; it is now `URL parse error: {0}`, naming the actual `url::ParseError` (e.g. `relative URL without a base`).
