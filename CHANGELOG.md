...
### Added
- `CreateSmartTransactionConfig::loaded_accounts_data_size_limit` and the same field on `CreateSmartTransactionSeedConfig` (both with `with_loaded_accounts_data_size_limit` builders), plus `Helius::get_compute_units_with_data_size_limit` / `get_compute_units_thread_safe_with_data_size_limit`. Sets the account-data budget a Transaction v1 carries in its message header, defaulting to `MAX_LOADED_ACCOUNTS_DATA_SIZE_BYTES` (64 MiB) — the implicit budget legacy and v0 transactions get. The limit is applied to the simulated draft as well as the final message, since simulation loads accounts too and an estimate taken under a larger budget does not hold on-chain. `CreateSmartTransactionConfig` also gains `with_lookup_tables`, `with_fee_payer`, `with_priority_fee_cap`, `with_cu_buffer_multiplier`, and `with_priority_fee_lamports_cap`. **Minor breaking change:** code constructing either config with a struct literal that enumerates every field must add the new one; literals ending in `..Default::default()` (including every example and test in this repo) are unaffected.
- `MAX_COMPUTE_UNIT_LIMIT` (1,400,000) and `MIN_COMPUTE_UNIT_LIMIT` (1,000) constants, replacing four bare `1_400_000` literals.

### Fixed
- **The requested compute-unit limit could exceed the protocol maximum.** The buffered estimate was floored at 1,000 but never capped: simulation is limited to 1,400,000 compute units, so the default 1.25x safety buffer produced up to 1,750,000 for any transaction consuming more than ~1,120,000 units, and a caller-supplied `cu_buffer_multiplier` could push any transaction over. Neither `v1::Message::try_compile_with_config` nor `v1::Message::validate` rejects an out-of-range limit, so the SDK signed and submitted transactions the cluster refuses. The requested limit is now capped at `MAX_COMPUTE_UNIT_LIMIT` in all three builders (the existing `MIN_COMPUTE_UNIT_LIMIT` floor behaviour is unchanged, including that the buffer is not applied below it) (`create_smart_transaction`, `create_smart_transaction_with_seeds`, `create_smart_transaction_without_signers`), on both the legacy/v0 `ComputeBudget` path and the v1 header-config path. A non-finite or non-positive multiplier now falls back to the default instead of producing a garbage limit.
